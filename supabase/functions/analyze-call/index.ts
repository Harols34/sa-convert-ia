...
serve(async (req) => {
  ...
  try {
    ...
    // Get the call ID and selected behaviors from the request body
    const { callId, selectedBehaviors } = await req.json();
    ...
    // Get behaviors - either selected ones or all active ones
    ...
    // If specific behaviors are selected, filter by those IDs
    if (selectedBehaviors && Array.isArray(selectedBehaviors) && selectedBehaviors.length > 0) {
      behaviorsQuery = behaviorsQuery.in('id', selectedBehaviors);
      console.log(`Filtering by ${selectedBehaviors.length} selected behaviors`);
    }
    ...
    if (!behaviors || behaviors.length === 0) {
      const errorMessage = selectedBehaviors && selectedBehaviors.length > 0
        ? "Los comportamientos seleccionados no están disponibles o activos"
    ...
    console.log(`Found ${behaviors.length} behaviors for analysis (account: ${call.account_id})`);
    if (selectedBehaviors) {
      console.log(`Using selected behaviors: ${behaviors.map(b => b.name).join(', ')}`);
    }
