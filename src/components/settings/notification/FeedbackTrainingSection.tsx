...
export default function FeedbackTrainingSection({
  ...
  const getTrainingRecommendations = (score: number) => {
    if (score < 70) {
      return ["Capacitación en protocolos de atención al cliente", "Refuerzo en técnicas de comunicación efectiva", "Seguimiento del guion de llamada"];
    } else if (score < 85) {
      return ["Mejora en técnicas de negociación", "Refuerzo en conocimiento de productos"];
    } else {
      return ["Desarrollo de habilidades de liderazgo", "Capacitación para ser mentor de otros agentes"];
    }
  };
...
Generado el ${new Date().toLocaleDateString()} | ID Plan: TRAINING-${agent.id.substring(0, 8)}`;
  ...
  const handleDownloadPlan = () => {
    ...
    const file = new Blob([trainingPlan], {
      type: 'text/plain'
    });
  ...
  const handleGenerateFullReport = async () => {
    ...
    try {
      ...
      toast.success("Reporte completo generado y guardado", {
        id: "generate-report"
      });
    } catch (error) {
  ...
  return <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Select defaultValue={selectedDays.toString()} onValueChange={value => onChangeDateRange(parseInt(value))}>
        ...
        <CardContent>
          {loadingReports ? <div className="flex justify-center p-4">
              <Loader2 className="h-6 w-6 animate-spin text-primary" />
            </div> : <ScrollArea className="h-64">
              <div className="space-y-4">
                {groupedAgents.map((agent, idx) => <div key={idx} className="border-b pb-4 last:border-0">
                    <h4 className="font-medium">{agent.name}</h4>
                    <div className="flex items-center mt-1 mb-2">
                      <span className="text-sm text-muted-foreground">{agent.totalCalls} llamadas totales</span>
                      <span className="mx-2">•</span>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${agent.averageScore >= 80 ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : agent.averageScore >= 60 ? 'bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-100' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100'}`}>
                    ...
                    <div className="text-sm">
                      ...
                      <ul className="list-disc pl-5">
                        {getTrainingRecommendations(agent.averageScore).map((rec, idx) => <li key={idx}>{rec}</li>)}
                      </ul>
                    </div>
                    
                    <div className="flex justify-end mt-2">
                      <Button variant="outline" size="sm" onClick={() => generateTrainingPlan(agent)}>
          ...
          <div className="mt-4 flex justify-end">
            <Button variant="outline" size="sm" onClick={handleGenerateFullReport} disabled={groupedAgents.length === 0}>
      ...
      <Dialog open={planOpen} onOpenChange={setPlanOpen}>
        <DialogContent className="max-w-2xl">
          ...
          {generatingPlan ? <div className="flex flex-col items-center justify-center py-8">
              <Loader2 className="h-8 w-8 animate-spin mb-4 text-primary" />
              <p className="text-sm text-muted-foreground">Generando plan personalizado...</p>
            </div> : <Textarea value={trainingPlan} readOnly rows={15} className="font-mono text-sm" />}
          
          <DialogFooter>
            <Button onClick={handleDownloadPlan} disabled={generatingPlan || !trainingPlan} className="flex items-center gap-2">
